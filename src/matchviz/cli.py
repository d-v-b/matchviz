...
@click.option(
    '--matches-csv', 
    help='Filename of a CSV file containing summarized matches',
    type=click.STRING)
@click.option(
    '--bigstitcher-xml', 
    type=click.STRING,
...
@click.option(
    ...
    required=True,
    type=click.STRING)
@click.option('--dataset-name', required=True, type=click.STRING)
@click.option('--invert-y-axis', type=click.BOOL, default=False)
@click.option('--invert-x-axis', type=click.BOOL, default=False)
def plot_matches_cli(
    ...
    if matches_csv is not None:
        if bigstitcher_xml is not None:
            raise ValueError('You may only specify matches_csv or bigstitcher_xml, not both.')
        data = pl.read_csv(matches_csv)
    elif bigstitcher_xml is not None:
        bigstitcher_xml_normalized = parse_url(bigstitcher_xml)
        data = fetch_summarize_matches(bigstitcher_xml=bigstitcher_xml_normalized, pool=pool, anon=anon)
    fig = plot_matches(
        data=data,
...
def tabulate_matches_cli(
    bigstitcher_xml: str, output: Literal["csv"] | None
):
    """
