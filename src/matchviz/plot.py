...
def plot_matches(
    ...
    for row in data.rows():
        ...
        if (id_self, id_other) not in completed_pairs:
            ...
            coords_other = rows_other.select(pl.first('x_coord_self'), pl.first('y_coord_self')).to_numpy()[0]
            name_other = rows_other.select(pl.first('image_name')).item()
            ...
            axs.text(
                line_x[0] * .65 + line_x[1] * .35, 
                line_y[0] * .65 + line_y[1] * .35, 
                row_model['num_matches'], 
                horizontalalignment='center',
                verticalalignment='top',
                rotation_mode='anchor'
                )
    axs.title.set_text(f'Number of matches found across tiles in {dataset_name}')
    fig.legend(loc=8, mode='expand', ncols=2, markerscale=.5)
    fig.tight_layout()
