...
def array_to_hoaffine(
        array: np.ndarray,
        dimensions: tuple[str, ...]) -> pydantic_bigstitcher.transform.HoAffine[Any]:
...
def apply_hoaffine(
    ...
    if data.shape[-1] == len(tx.translation):
        # convert input points to homogenous form
        points_parsed = np.concatenate((data, np.ones((data.shape[0], 1))), axis=1)
        result = (hoaffine_to_array(tx, dimensions=dimensions) @ points_parsed.T).T[:,:-1]
    else:
        points_parsed = data
